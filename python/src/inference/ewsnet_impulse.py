...
class EWSNet():
    ...
    def finetune(self,X,y, freeze_feature_extractor=True, learning_rate = 5e-5, batch_size = 512, tune_epochs = 5, models_from=0, models_till=-1):
        """
        ...
        """
        models_till = self.ensemble if models_till < 0 else models_till
        if(len(X.shape)==2):
            X = np.expand_dims(X,axis=1)
        X_train, X_val, y_train, y_val = train_test_split(X, y, test_size=0.05, random_state=1)
        train_dataset   = tf.data.Dataset.from_tensor_slices((X_train, y_train)).batch(batch_size)
        val_dataset     = tf.data.Dataset.from_tensor_slices((X_val, y_val)).batch(batch_size)
        for i in range(models_from, min(self.ensemble,models_till)):
    ...
    def load_model(self,weight_dir,prefix,suffix):
        ...
        for i in range(self.ensemble):
            if(os.path.exists("{}/{}{}{}".format(weight_dir,prefix,i,suffix))):
                print(f"Loading Model in Ensemble:  {i+1}/{len(self.model)}")
                # loaded_model = tf.keras.models.load_model("{}/{}{}{}".format(weight_dir,prefix,i,suffix))
                # print(loaded_model.summary())
                self.model[i] = tf.keras.models.load_model("{}/{}{}{}".format(weight_dir,prefix,i,suffix))
            else:
              print(f"Failed loading weights:  {i+1}/{len(self.model)}. File not found: {weight_dir}/{prefix}{i}{suffix}")
