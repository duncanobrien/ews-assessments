...
#' @param sensitivity. Numeric value. For rolling or EWSNet estimates, \code{sensitivity} should equal the minimum Tau correlation or EWSNet prediction probability that constitutes a 'warning'. For rolling window EWSs, \code{sensitivity} is the number of consecutive 'signals' that constitutes a 'warning'. If \code{NULL} and method = \code{"ML"}, the highest probability is the prediction
#' @param outcome A two column dataframe or character matrix. The first column is a vector of system reference that matches that in \code{ews.data} (e.g. \code{"Kinneret_Monthly"}). The second column is a character vector of outcomes - either \code{"trans"} for a non-linearly transitioning system of \code{"no.trans"} for system that does not non-linearly transition.
