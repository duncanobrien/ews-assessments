...
labels <- c(-3,round(brms::logit_scaled(c(0.25,0.5,0.75)),1),3) #ensure breaks @ 0.25 probability intervals
breaks <- c(-3,round(brms::logit_scaled(c(0.25,0.5,0.75)),1),3)
...
ggsave(ggplot(dat_method_trials,aes(y = .variable, x = .value)) +
         ...
         scale_y_discrete(labels = c("univariate rolling","EWSNet","multivariate rolling","univariate expanding","multivariate expanding"))+
         facet_wrap(~res)+
         theme_bw()+
