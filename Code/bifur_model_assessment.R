...
kin_density_yr <- data.frame(as.numeric(kin_yr_dat$Date[kin_yr_dat$Date >1980]),rowSums(kin_yr_dat[kin_yr_dat$Date >1980,2:50]))
kin_density_yr <- data.frame(seq_along(as.numeric(kin_yr_dat$Date)),rowSums(kin_yr_dat[,2:50]))
...
wind_density_yr <- data.frame(seq_along(wind_yr_dat$Date[wind_yr_dat$Date > 1984]),rowSums(wind_yr_dat[wind_yr_dat$Date > 1984,2:23]))
wind_density_yr <- data.frame(seq_along(wind_yr_dat$Date),rowSums(wind_yr_dat[,2:23]))
...
kas_density_yr <- data.frame(seq_along(kas_yr_dat$date[kas_yr_dat$date<1997]),rowSums(kas_yr_dat[kas_yr_dat$date<1997,2:89]))
...
UZ_density <- data.frame(seq_along(UZ_mth_dat$date),rowSums(UZ_mth_dat[,2:79]))
UZ_density_yr <- data.frame(seq_along(UZ_yr_dat$date[UZ_yr_dat$date<1992]),rowSums(UZ_yr_dat[UZ_yr_dat$date<1992,2:79]))
...
mon_density <- data.frame(seq_along(mon_mth_dat$date),rowSums(mon_mth_dat[,2:107]))
mon_density_yr <- data.frame(seq_along(mon_yr_dat$date[mon_yr_dat$date<2013]),rowSums(mon_yr_dat[mon_yr_dat$date<2013,2:107]))

wash_density <- data.frame(seq_along(wash_mth_dat$date),rowSums(wash_mth_dat[,2:16]))
wash_density_yr <- data.frame(seq_along(wash_yr_dat$date[wash_yr_dat$date<1975]),rowSums(wash_yr_dat[wash_yr_dat$date<1975,2:16]))

####################################################################################################
## Observed Fits
####################################################################################################
kinOU <- fit_bif_mod(kin_density_yr,model = "OU")
kinSN <- fit_bif_mod(kin_density_yr,model = "LSN")
kinTC <- fit_bif_mod(kin_density_yr,model = "LTC")
kinsuperPF <- fit_bif_mod(kin_density_yr,model = "superLPF")
kinsubPF <- fit_bif_mod(kin_density_yr,model = "subLPF")
...
windOU <- fit_bif_mod(wind_density_yr,model = "OU")
windSN <- fit_bif_mod(wind_density_yr,model = "LSN")
windTC <- fit_bif_mod(wind_density_yr,model = "LTC")
windsuperPF <- fit_bif_mod(wind_density_yr,model = "superLPF")
windsubPF <- fit_bif_mod(wind_density_yr,model = "subLPF")
...
madOU <- fit_bif_mod(mad_density_yr,model = "OU")
madSN <- fit_bif_mod(mad_density_yr,model = "LSN")
madTC <- fit_bif_mod(mad_density_yr,model = "LTC")
madsuperPF <- fit_bif_mod(mad_density_yr,model = "superLPF")
madsubPF <- fit_bif_mod(mad_density_yr,model = "subLPF")
...
kasOU <- fit_bif_mod(kas_density_yr,model = "OU")
kasSN <- fit_bif_mod(kas_density_yr,model = "LSN")
kasTC <- fit_bif_mod(kas_density_yr,model = "LTC")
kassuperPF <- fit_bif_mod(kas_density_yr,model = "superLPF")
kassubPF <- fit_bif_mod(kas_density_yr,model = "subLPF")
...
LZOU <- fit_bif_mod(LZ_density_yr,model = "OU")
LZSN <- fit_bif_mod(LZ_density_yr,model = "LSN")
LZTC <- fit_bif_mod(LZ_density_yr,model = "LTC")
LZsuperPF <- fit_bif_mod(LZ_density_yr,model = "superLPF")
LZsubPF <- fit_bif_mod(LZ_density_yr,model = "subLPF")
...
UZOU <- fit_bif_mod(UZ_density_yr,model = "OU")
UZSN <- fit_bif_mod(UZ_density_yr,model = "LSN")
UZTC <- fit_bif_mod(UZ_density_yr,model = "LTC")
UZsuperPF <- fit_bif_mod(UZ_density_yr,model = "superLPF")
UZsubPF <- fit_bif_mod(UZ_density_yr,model = "subLPF")
...
washOU <- fit_bif_mod(wash_density_yr,model = "OU")
washSN <- fit_bif_mod(wash_density_yr,model = "LSN")
washTC <- fit_bif_mod(wash_density_yr,model = "LTC")
washsuperPF <- fit_bif_mod(wash_density_yr,model = "superLPF")
washsubPF <- fit_bif_mod(wash_density_yr,model = "subLPF")
...
leveOU <- fit_bif_mod(leve_density_yr,model = "OU")
leveSN <- fit_bif_mod(leve_density_yr,model = "LSN")
leveTC <- fit_bif_mod(leve_density_yr,model = "LTC")
levesuperPF <- fit_bif_mod(leve_density_yr,model = "superLPF")
levesubPF <- fit_bif_mod(leve_density_yr,model = "subLPF")
...
monOU <- fit_bif_mod(mon_density_yr,model = "OU")
monSN <- fit_bif_mod(mon_density_yr,model = "LSN")
monTC <- fit_bif_mod(mon_density_yr,model = "LTC")
monsuperPF <- fit_bif_mod(mon_density_yr,model = "superLPF")
monsubPF <- fit_bif_mod(mon_density_yr,model = "subLPF")
...
## Simulate Replicates
####################################################################################################
kin.repsSN <- pbmcapply::pbmclapply(1:2000, function(i){compare(kinOU, kinSN)},mc.cores=9)
kin.repsTC <- pbmcapply::pbmclapply(1:2000, function(i){compare(kinOU, kinTC)},mc.cores=9)
kin.repssubPF <- pbmcapply::pbmclapply(1:2000, function(i){compare(kinOU, kinsubPF)},mc.cores=9)

kin.lrSN <- lik_ratios(kin.repsSN) |> dplyr::mutate(model = "SN")
kin.lrTC <- lik_ratios(kin.repsTC) |> dplyr::mutate(model = "TC")
kin.lrPF <- lik_ratios(kin.repssubPF) |> dplyr::mutate(model = "PF")

kin.observed.dat <- data.frame("model" = c("SN","TC","PF"),
                           "obs" = c(-2*(kinOU$loglik - kinSN$loglik),-2*(kinOU$loglik - kinTC$loglik),-2*(kinOU$loglik - kinsubPF$loglik)))
...
wind.observed.dat <- data.frame("model" = c("SN","TC","PF"),
                               "obs" = c(-2*(windOU$loglik - windSN$loglik),-2*(windOU$loglik - windTC$loglik),-2*(windOU$loglik - windsubPF$loglik)))

wind_lr <- dplyr::left_join(rbind(wind.lrSN,wind.lrTC,wind.lrPF),wind.observed.dat,by="model") |>
  dplyr::mutate(lake = "Windermere")

wind_lr <- dplyr::left_join(rbind(wind.lrSN,wind.lrTC|>
                                    dplyr::filter(value > -2e+02 & value < 2e+02),
                                  wind.lrPF |>
                                   dplyr::filter(value > -2e+02 & value < 2e+02)),wind.observed.dat,by="model") |>
  dplyr::mutate(lake = "Windermere")

ggplot(rbind(wind_lr)) + 
  geom_density(aes(value, fill=simulation), alpha=0.8,size=0.3) + 
  geom_vline(aes(xintercept=obs),linetype = "dashed",col="grey20") + 
  scale_fill_manual(values = c(c("#A1B4FE","#FFE7A1")))+
...
mad.repsSN <- pbmcapply::pbmclapply(1:2000, function(i){compare(madOU, madSN)},mc.cores=9)
mad.repsTC <- pbmcapply::pbmclapply(1:2000, function(i){compare(madOU, madTC)},mc.cores=9)
mad.repssuperPF <- pbmcapply::pbmclapply(1:2000, function(i){compare(madOU, madsuperPF)},mc.cores=9)

mad.lrSN <- lik_ratios(mad.repsSN) |> dplyr::mutate(model = "SN")
mad.lrTC <- lik_ratios(mad.repsTC) |> dplyr::mutate(model = "TC")
mad.lrPF <- lik_ratios(mad.repssubPF) |> dplyr::mutate(model = "PF")
...
kas.repsSN <- pbmcapply::pbmclapply(1:2000, function(i){compare(kasOU, kasSN)},mc.cores=9)
kas.repsTC <- pbmcapply::pbmclapply(1:2000, function(i){compare(kasOU, kasTC)},mc.cores=9)
kas.repssuperPF <- pbmcapply::pbmclapply(1:2000, function(i){compare(kasOU, kassuperPF)},mc.cores=9)
...
LZ.repsSN <- pbmcapply::pbmclapply(1:2000, function(i){compare(LZOU, LZSN)},mc.cores=9)
LZ.repsTC <- pbmcapply::pbmclapply(1:2000, function(i){compare(LZOU, LZTC)},mc.cores=9)
LZ.repssubPF <- pbmcapply::pbmclapply(1:2000, function(i){compare(LZOU, LZsubPF)},mc.cores=9)
...
UZ.repsSN <- pbmcapply::pbmclapply(1:2000, function(i){compare(UZOU, UZSN)},mc.cores=9)
UZ.repsTC <- pbmcapply::pbmclapply(1:2000, function(i){compare(UZOU, UZTC)},mc.cores=9)
UZ.repssubPF <- pbmcapply::pbmclapply(1:2000, function(i){compare(UZOU, UZsubPF)},mc.cores=9)

UZ.lrSN <- lik_ratios(UZ.repsSN) |> dplyr::mutate(model = "SN")
UZ.lrTC <- lik_ratios(UZ.repsTC) |> dplyr::mutate(model = "TC")
UZ.lrPF <- lik_ratios(UZ.repssubPF) |> dplyr::mutate(model = "PF")
...
wash.repsSN <- pbmcapply::pbmclapply(1:2000, function(i){compare(washOU, washSN)},mc.cores=9)
wash.repsTC <- pbmcapply::pbmclapply(1:2000, function(i){compare(washOU, washTC)},mc.cores=9)
wash.repssubPF <- pbmcapply::pbmclapply(1:2000, function(i){compare(washOU, washsubPF)},mc.cores=9)

wash.lrSN <- lik_ratios(wash.repsSN) |> dplyr::mutate(model = "SN")
wash.lrTC <- lik_ratios(wash.repsTC) |> dplyr::mutate(model = "TC")
wash.lrPF <- lik_ratios(wash.repssubPF) |> dplyr::mutate(model = "PF")
...
leve.repsSN <- pbmcapply::pbmclapply(1:2000, function(i){compare(leveOU, leveSN)},mc.cores=9)
leve.repsTC <- pbmcapply::pbmclapply(1:2000, function(i){compare(leveOU, leveTC)},mc.cores=9)
leve.repssuperPF <- pbmcapply::pbmclapply(1:2000, function(i){compare(leveOU, levesuperPF)},mc.cores=9)

leve.lrSN <- lik_ratios(leve.repsSN) |> dplyr::mutate(model = "SN")
leve.lrTC <- lik_ratios(leve.repsTC) |> dplyr::mutate(model = "TC")
leve.lrPF <- lik_ratios(leve.repssubPF) |> dplyr::mutate(model = "PF")
...
mon.repsSN <- pbmcapply::pbmclapply(1:2000, function(i){compare(monOU, monSN)},mc.cores=9)
mon.repsTC <- pbmcapply::pbmclapply(1:2000, function(i){compare(monOU, monTC)},mc.cores=9)
mon.repssubPF <- pbmcapply::pbmclapply(1:2000, function(i){compare(monOU, monsubPF)},mc.cores=9)

mon.lrSN <- lik_ratios(mon.repsSN) |> dplyr::mutate(model = "SN")
mon.lrTC <- lik_ratios(mon.repsTC) |> dplyr::mutate(model = "TC")
mon.lrPF <- lik_ratios(mon.repssubPF) |> dplyr::mutate(model = "PF")
...
find_quartile(x=rbind(kin.lrSN,kin.lrTC,kin.lrPF),obs=kin.observed.dat)$outcome
find_quartile(x=rbind(kas.lrSN,kas.lrTC,kas.lrPF),obs=kas.observed.dat)$outcome
find_quartile(x = rbind(LZ.lrSN,LZ.lrTC,LZ.lrPF),obs = LZ.observed.dat)$outcome
find_quartile(x=rbind(mad.lrSN,mad.lrTC,mad.lrPF),obs = mad.observed.dat)$outcome
find_quartile(x = rbind(wind.lrSN,wind.lrTC,wind.lrPF),obs = wind.observed.dat)
