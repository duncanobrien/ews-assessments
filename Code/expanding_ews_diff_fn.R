...
extract.ews.difference <- function(ews.data, sensitivity, outcome, method,surrogate = FALSE){
  ...
  out <-  as.data.table(ews.data)
  
  if(!("data_source" %in% colnames(out)) & meth != "expanding"){
    out <- cbind("data_source" = NA,out)
  }
  ...
  if(meth == "expanding"){
    ...
    }else{ #repeat but ignoring data_source column if none given
      out <- out[,first.ews := time[rep(rle(threshold.crossed)$lengths >= sensitivity & 
                                          rle(threshold.crossed)$value == 1,
                                        times = rle(threshold.crossed)$lengths)][1], 
    ...
    out <- out %>% # match warning prediction to ground truth provided in 'outcome' argument
      .[,prediction := ifelse(!is.na(first.ews) & outcome[outcome[,1] %in% paste(lake,res,sep = "_"),2] == "no.trans","unknown",
                              ifelse(is.na(interval.prior) & is.na(first.ews) & outcome[outcome[,1] %in% paste(lake,res,sep = "_"),2] == "trans", "miss",   
                                     ifelse((is.na(interval.prior) & is.na(first.ews) & outcome[outcome[,1] %in% paste(lake,res,sep = "_"),2] == "no.trans")|interval.prior == 0, "match", 
                                            ifelse(interval.prior > 0 & outcome[outcome[,1] %in% paste(lake,res,sep = "_"),2] == "trans", "prior", "post")))),
    ...
    out <- out %>%   
      data.table::melt(., id.vars = c("data_source","lake","res","method","computation"),
                       measure.vars = colnames(out)[!grepl("data_source|res|lake|method|computation", colnames(out))],
    ...
    .[,threshold.crossed := ifelse(metric.score > sensitivity,1, 0), 
      by = c("data_source","metric.code","lake","res")] %>% # identify warnings when correlation exceed `sensitivity`
    .[,prediction := ifelse(threshold.crossed == 0 & outcome[outcome[,1] %in% paste(lake,res,sep = "_"),2] == "no.trans","match",
                            ifelse(threshold.crossed == 0 & outcome[outcome[,1] %in% paste(lake,res,sep = "_"),2] == "trans", "miss",   
                                   ifelse(threshold.crossed == 1 & outcome[outcome[,1] %in% paste(lake,res,sep = "_"),2] == "no.trans", "unknown","match"))),
      by = c("lake","res","metric.code")] # match warning prediction to ground truth
    }
    if(isTRUE(surrogate)){
      
      out <- out %>%  
          .[,perm := rep(c("obs","perm")),by = c("data_source","lake","res")] %>%
          data.table::melt(.,measure.vars = colnames(out)[!grepl("data_source|res|lake|method|computation|perm", colnames(out))],
                           variable.name = "metric.code", value.name = "metric.score") %>%
          data.table::dcast(., data_source + lake + res + metric.code + method + computation~ perm, value.var = "metric.score") %>%
        .[,threshold.crossed := ifelse(obs > 0 & perm >= sensitivity,1, 0), 
            by = c("data_source","perm","metric.code","lake","res")] %>%
        .[,prediction := ifelse(threshold.crossed == 0 & outcome[outcome[,1] %in% paste(lake,res,sep = "_"),2] == "no.trans","match",
                                ifelse(threshold.crossed == 0 & outcome[outcome[,1] %in% paste(lake,res,sep = "_"),2] == "trans", "miss",   
                                       ifelse(threshold.crossed == 1 & outcome[outcome[,1] %in% paste(lake,res,sep = "_"),2] == "no.trans", "unknown","match"))),
          by = c("lake","res","metric.code")]
...
#out <- genus_lake_ewsnet
    out <- out %>%  
      ...
      # tmp <- out[pos_outcome == ewsnet_prediction,] %>% # on subset of data (the strongest prediction for each time series)
      # .[,prediction := ifelse(threshold.crossed == 0 & ewsnet_prediction != "critical" & outcome[outcome[,1] %in% paste(lake,res,sep = "_"),2] == "no.trans","match",
      #                         ifelse(threshold.crossed == 0 & ewsnet_prediction == "critical" & outcome[outcome[,1] %in% paste(lake,res,sep = "_"),2] == "trans", "miss",   
      #                                ifelse(threshold.crossed == 1 & ewsnet_prediction == "critical" & outcome[outcome[,1] %in% paste(lake,res,sep = "_"),2] == "no.trans", "unknown",
      #                                       ifelse(threshold.crossed == 1 & ewsnet_prediction != "critical" & outcome[outcome[,1] %in% paste(lake,res,sep = "_"),2] == "trans", "miss",
      #                                              ifelse(threshold.crossed == 0 & ewsnet_prediction != "critical" & outcome[outcome[,1] %in% paste(lake,res,sep = "_"),2] == "trans", "miss",                                                   
      #                                                     "match"))))),
      #                        by = c("lake","res","scaling")] # match warning prediction to ground truth
       
      tmp <- out[pos_outcome == ewsnet_prediction,] %>% # on subset of data (the strongest prediction for each time series)
        .[,prediction := ifelse(threshold.crossed == 0  & outcome[outcome[,1] %in% paste(lake,res,sep = "_"),2] == "no.trans", "match",
                                ifelse(threshold.crossed == 0 & ewsnet_prediction == "critical" & outcome[outcome[,1] %in% paste(lake,res,sep = "_"),2] == "trans", "miss",   
                                       ifelse(threshold.crossed == 1 & ewsnet_prediction == "critical" & outcome[outcome[,1] %in% paste(lake,res,sep = "_"),2] == "no.trans", "unknown",
                                              ifelse(threshold.crossed == 1 & ewsnet_prediction == "critical" & outcome[outcome[,1] %in% paste(lake,res,sep = "_"),2] == "trans", "match",
                                                            ifelse(threshold.crossed == 1 & ewsnet_prediction == "smooth" & outcome[outcome[,1] %in% paste(lake,res,sep = "_"),2] == "no.trans","match",
                                                            "miss"))))),
    ...
    out <- out[tmp, on =  c("data_source","lake","res","scaling"), prediction := i.prediction] # merge tmp subset to main data.table and fill missing values
