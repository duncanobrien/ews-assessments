...
#' Linearized Pitchfork model
#'
#' Estimate the mean and variance of the linearized pitchfork
#' process (in which the control constant changes over time)
...
LPF <- function(Xo, to, t1, pars){
  ...
  if(is.na(check) | check | pars[3] < 0){
    ...
    moments <- function(t,y,p){ 
      ...
      yd2 <- -2*r_t*2*r_t*(y[1] - p[3]-sqrtR)*y[2] + p[4]^2*(p[3] + sqrtR)
