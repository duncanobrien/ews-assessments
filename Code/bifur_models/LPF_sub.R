...
#' Linearized Subcritical Pitchfork model
#'
#' Estimate the mean and variance of the linearized subcritical 
...
subLPF<- function(Xo, to, t1, pars){
  ...
  if(is.na(check) | check | pars[3] < 0){
    ...
    moments <- function(t,y,p){ 
      r_t <- R(t,p)
      sqrtR <- sqrt(r_t) 
      yd1 <-  2*(r_t^(3/2)) - 4*r_t*(y[1]-p[3]+sqrtR)
      yd2 <-  -4*r_t*( 2*(r_t^(3/2)) - 4*r_t*(y[1]-p[3]+sqrtR))*y[2] + p[4]^2*(p[3] - sqrtR)
